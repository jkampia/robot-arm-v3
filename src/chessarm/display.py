...
class DisplayTile:
    ...
    def __init__(self):
        ...
        self.data = np.full((self.tileH, self.tileW, 3), COLOR_RGB["black"], dtype=np.uint8)
    ...
    def toBase64(self):
        ...
        if not isinstance(self.data, np.ndarray):
            raise TypeError("DisplayTile.data must be a numpy array")

        if self.data.dtype != np.uint8:
            raise ValueError("Image dtype must be uint8")

        success, buffer = cv2.imencode(".jpg", self.data)
...
class ChessarmDisplay:
    ...
    def __init__(self):
        ...
        self.displayTiles = [DisplayTile() for _ in range(self.numTiles)]
    ...
    def updateAllDisplayTiles(self):
        # defines where each displayTile pulls its data from 
        originalFrame = self.frameRequester.requestFrameToMat()
        
        if originalFrame is not None:
            
            self.displayTiles[0].data = originalFrame.copy()
            ...
            self.displayTiles[1].data = self.embedInTile(rectifiedFrame)
